...
void PersonSetDestroy(PersonSet **pps) {
  ...
  // destroy each Person stored in the array
  free((*pps)->array);
  free(*pps);
  *pps = NULL;
...
static void append(PersonSet *ps, Person *p) {
  ...
  if (ps->size == ps->capacity) {
    ps->capacity *=2;
    ps->array = (Person**)realloc(ps->array, ps ->capacity * sizeof(Person *)); //Incompleto?
    if (ps->array == NULL) {
...
int PersonSetEquals(const PersonSet *ps1, const PersonSet *ps2) {
  ...
  //1º Ver se tem o mesmo número de elementos, depois usar o subset, se for subconjunto do primeiro e tem o mesmo número de elementos então é igual
  // COMPLETE ...
